#![no_std]
use soroban_sdk::{contract, contractimpl, contracterror, symbol_short, Address, Env, Map, String, Symbol, Vec};
...
impl Contract {
    ...
    pub fn issue_certificate(
        ...
        let mut certificates: Map<String, CertificateDetails> = env.storage().instance().get(&CERTIFICATES).unwrap();
    ...
    pub fn get_certificate_details(env: Env, cert_id: String) -> Result<CertificateDetails, Error> {
        // Get the certificates map
        let certificates: Map<String, CertificateDetails> = env.storage().instance().get(&CERTIFICATES).unwrap();
    ...
    pub fn verify_certificate(env: Env, cert_id: String, metadata_hash: String) -> Result<bool, Error> {
